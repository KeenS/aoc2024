...
def canBeCalibratedAux(result: Nat) (data: List Nat) (acc: Nat) (op: Nat -> Nat -> Nat): Bool :=
  match data with
  | [] => result == acc
  | n::ns => acc <= result && (canBeCalibratedAux result ns (op acc n) (. + .) || canBeCalibratedAux result ns (op acc n) (. * .))

def canBeCalibrated (input: (Nat × List Nat)): Bool :=
  let (result, data) := input
  canBeCalibratedAux result data 0 (. + .) || canBeCalibratedAux result data 1 (. * .)

#check canBeCalibrated (190, [10, 19])
#check canBeCalibrated (3267, [81, 40, 27])
#check not $ canBeCalibrated (83, [17, 5])
